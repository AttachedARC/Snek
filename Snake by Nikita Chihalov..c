...
fy++;
            ...
            else 
            {
              for(int i = 1; i<=ntail; i++)
              {
                 if (fx == tailX[i] && fy == tailY[i]) { fx = rand()% (window_max-3)+1; fy = rand()% (window_max-3)+2; food_eat = true; } // Если координаты совпали то
                 else food_eat = false;
              }
           }
         }
...
//== Отрисовка ==
       ...
       attrset(A_BOLD);
       mvaddch(hy, hx, '@' | COLOR_PAIR(3)| A_BOLD);//Голова
       for (int i = 1; i<ntail; i++)
       { 
          if (i<(ntail-1)) { mvaddch(tailY[i],tailX[i], '*' | COLOR_PAIR(3)| A_BOLD); }//Тельце
            else {  mvaddch(tailY[i],tailX[i], '+' | COLOR_PAIR(3)| A_BOLD); }//Хвостик
       }
//===========       
         refresh();
         draw_window(); //
...
//****************** 
/*    
init_color(COLOR_MAGENTA, 0, 0, 255);
